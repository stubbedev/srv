...
func WriteTraefikConfig(cfg *config.Config) error {
	...
	// Traefik watches this directory; a truncated read drops the routers.
	return fsutil.AtomicWriteFile(TraefikConfigPath(cfg), []byte(body), constants.FilePermDefault)
}
...
func prometheusYAML() string {
	...
	doc.ScrapeConfigs = []promScrapeConfig{{
		JobName: "traefik",
		StaticConfigs: []struct {
			Targets []string `yaml:"targets"`
