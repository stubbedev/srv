...
// =============================================================================
// generatePHPDockerfile (FrankenPHP runtime — extra coverage on top of
// php_render_test.go)
// =============================================================================

func TestGeneratePHPDockerfile_SpecificVersion(t *testing.T) {
	df := generatePHPDockerfile(&PHPSiteInfo{
		PHPVersion: "8.3",
		Extensions: []string{"pdo_mysql"},
	})
	if !strings.Contains(df, "FROM dunglas/frankenphp:php8.3-alpine") {
		t.Errorf("expected FROM dunglas/frankenphp:php8.3-alpine, got:\n%s", df)
	}
}
