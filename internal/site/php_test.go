...
func TestPHPImageFingerprint_Deterministic(t *testing.T) {
	a := PHPImageFingerprint(&PHPSiteInfo{PHPVersion: "8.4", Extensions: []string{"redis", "bcmath", "gd"}})
	b := PHPImageFingerprint(&PHPSiteInfo{PHPVersion: "8.4", Extensions: []string{"gd", "redis", "bcmath"}})
	if a != b {
		t.Errorf("fingerprint should be order-insensitive: %q vs %q", a, b)
	}
}

func TestPHPImageFingerprint_DiffersByVersion(t *testing.T) {
	a := PHPImageFingerprint(&PHPSiteInfo{PHPVersion: "8.3", Extensions: []string{"redis"}})
	b := PHPImageFingerprint(&PHPSiteInfo{PHPVersion: "8.4", Extensions: []string{"redis"}})
	if a == b {
		t.Errorf("fingerprint should change with PHP version: %q == %q", a, b)
	}
}

func TestPHPImageFingerprint_DiffersByExtensions(t *testing.T) {
	a := PHPImageFingerprint(&PHPSiteInfo{PHPVersion: "8.4", Extensions: []string{"redis"}})
	b := PHPImageFingerprint(&PHPSiteInfo{PHPVersion: "8.4", Extensions: []string{"redis", "imagick"}})
	if a == b {
		t.Errorf("fingerprint should change with extension set: %q == %q", a, b)
	}
}
...
func TestGeneratePHPDockerfile_LatestVersion(t *testing.T) {
	...
	if !strings.Contains(df, "# Generated by srv") {
		t.Error("Dockerfile should contain generated comment")
	}
	if !strings.HasPrefix(df, "# syntax=docker/dockerfile:") {
		t.Error("Dockerfile should declare buildkit syntax for cache mounts")
	}
