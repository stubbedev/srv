...
func generateStaticContainerName(name string) string {
	...
	return fmt.Sprintf("srv_static_%s", shortHash)
}

// staticNginxConf is the nginx configuration for static sites.
// It blocks access to sensitive files and directories.
const staticNginxConf = `# Generated by srv - static site nginx config
server {
    listen 80;
    server_name _;
...
func WriteStaticSiteCompose(dir, name, domain, networkName string, isLocal bool) error {
	// Write nginx config
	nginxConfPath := filepath.Join(dir, "nginx.conf")
	if err := os.WriteFile(nginxConfPath, []byte(staticNginxConf), 0o644); err != nil {
...
# To customize, edit nginx.conf and restart the site.
...
// RemoveStaticSiteCompose removes the generated docker-compose.yml and nginx.conf for static sites.
func RemoveStaticSiteCompose(dir string) error {
	if !IsStaticSite(dir) {
		return nil // Not a static site, don't remove
	}
	os.Remove(filepath.Join(dir, "nginx.conf"))
	composePath := filepath.Join(dir, "docker-compose.yml")
