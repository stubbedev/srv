...
// them) are replaced in place; a file the user has since edited is preserved and
// the fresh render lands beside it as "<name>.regenerated", reported in the
// returned warnings for the user to merge.
func WriteStaticSiteConfig(name string, meta SiteMetadata, force bool) (warnings []string, err error) {
	cfg, err := config.Load()
	if err != nil {
		return nil, err
	}
	...
	if err := os.MkdirAll(siteDir, constants.DirPermDefault); err != nil {
		return nil, fmt.Errorf("failed to create site config directory: %w", err)
	}
	recordedHashes := readGeneratedHashes(siteDir)
	...
	regenerated, err := writeOwnedFile(nginxConfPath, []byte(nginxConf), force, recordedHashes)
	if err != nil {
		return warnings, fmt.Errorf("failed to write nginx.conf: %w", err)
	}
	if regenerated {
		warnings = append(warnings, "nginx.conf has local modifications; fresh version written to nginx.conf.regenerated — merge your changes in and delete it")
	}
	...
	if err != nil {
		return warnings, fmt.Errorf("failed to marshal compose config: %w", err)
	}
	...
	regenerated, err = writeOwnedFile(composePath, []byte(content), force, recordedHashes)
	if err != nil {
		return warnings, fmt.Errorf("failed to write docker-compose.yml: %w", err)
	}
	if regenerated {
		warnings = append(warnings, "docker-compose.yml has local modifications; fresh version written to docker-compose.yml.regenerated — merge your changes in and delete it")
	}

	saveGeneratedHashes(siteDir, recordedHashes)
	return warnings, nil
}
