...
func WriteRoutesConfig(cfg *config.Config, set SiteRouteSet) error {
	...
	for _, r := range set.Routes {
		...
		if r.Rewrite != "" {
			...
			middlewares[mwName] = dynMiddleware{
				ReplacePathRegex: &dynReplacePathRegex{
					Regex:       r.PathRegex,
	...
	return fsutil.AtomicWriteFile(path, []byte(header+string(data)), constants.FilePermDefault)
}
