...
// DockerComposeTemplate renders traefik/docker-compose.yml. dnsUser and dnsPass
// are the dnsmasq HTTP credentials; they must be loaded or generated by the
// caller (see loadOrGenerateDNSCredentials) to avoid rotation on every call.
// The document is built from typed structs and marshalled once, so credentials
// and paths are encoded as YAML scalars — no string substitution into YAML, no
// injection surface.
//
...
// localhost).
func DockerComposeTemplate(networkName, sitesDir, dnsUser, dnsPass string) (string, error) {
	...
	dnsSvc := &composeService{
		Image:         docker.ImageDNS,
		ContainerName: docker.ContainerDNS,
		Restart:       "unless-stopped",
		Ports:         []string{"127.0.0.1:53:53/udp"},
		Environment: []string{
			"HTTP_USER=" + dnsUser,
			"HTTP_PASS=" + dnsPass,
		},
		...
		Logging: &composeLogging{Driver: "none"},
	}
...
func writeTraefikCompose(cfg *config.Config) error {
	...
	composeYML, err := DockerComposeTemplate(cfg.NetworkName, cfg.SitesDir, dnsUser, dnsPass)
	if err != nil {
		return err
	}
