...
// can break the document or inject keys, regardless of special characters.
...
// DockerComposeTemplate renders traefik/docker-compose.yml: the Traefik
// service plus nothing else — DNS is served by the srv daemon's embedded
// server (internal/dnsd), not a container, so there is no dns service to
// configure and no credentials to mount.
// The document is built from typed structs and marshalled once, so paths are
// encoded as YAML scalars — no string substitution into YAML, no injection
// surface.
//
// On Linux, Traefik uses network_mode: host to access localhost services
// directly. This lets 'srv proxy' reach services bound to 127.0.0.1 without
// code changes. The custom Docker network still backs container-to-container
// communication (containers connect to it and publish ports Traefik reaches via
// localhost).
func DockerComposeTemplate(networkName, sitesDir string) (string, error) {
	traefikSvc := &composeService{
		Image:         docker.ImageTraefik,
	...
	if platform.IsLinux() {
		// Host network mode gives Traefik direct access to localhost services;
		// it cannot publish ports or join custom networks.
	...
	doc := composeFile{
		...
		Networks: map[string]composeNetwork{
			"traefik": {Name: networkName, External: true},
		},
	}
...
func writeTraefikCompose(cfg *config.Config) error {
	composeYML, err := DockerComposeTemplate(cfg.NetworkName, cfg.SitesDir)
	if err != nil {
		return err
	}
	return fsutil.AtomicWriteFile(cfg.TraefikComposePath(), []byte(composeYML), constants.FilePermDefault)
}
...
func EnsureConfig(email string) error {
	...
	if err := fsutil.AtomicWriteFile(composePath, []byte(composeYML), constants.FilePermDefault); err != nil {
